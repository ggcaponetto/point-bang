 ...
 * changing the feel. Several buttons may share an edge (they press together,
 * exactly like a shared gamepad index) — there is no collision rule.
 * Order matters: tests (and the editor's mirror) pin these strings.
 */
function defProblems(d: ButtonDef): string[] {
  const problems: string[] = [];
  ...
  if (d.edge !== undefined && !normalizeEdge(d.edge))
    problems.push(`button ${d.id}: bad edge ignored (need left/right/top/bottom/any)`);
